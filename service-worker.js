const CACHE="vardiyacep-v1.7.0";
const APP_SHELL=["./","./index.html","./styles.css?v=1.7.0","./app.js?v=1.7.0","./manifest.webmanifest","./sample-data.json","./icons/icon-192.png","./icons/icon-512.png"];
...
async function checkReminder(){const data=await get("reminder");if(!data?.settings?.enabled||!data?.person)return;const now=new Date();const [h,m]=(data.settings.time||"20:00").split(":").map(Number);if(now.getHours()!==h||Math.abs(now.getMinutes()-m)>45)return;const key=`sent-${isoLocal(now)}`;if(await get(key))return;const tomorrow=addDays(now,1);const iso=isoLocal(tomorrow);const code=data.person.shifts?.[iso];if(!code)return;const map=infer(code,data.mappings);const predicted=(data.predictedDates||[]).includes(iso);const baseTitle=map.category==="morning"?"Yarın sabahçısın ☀️":map.category==="evening"?"Yarın akşam vardiyasındasın":map.category==="night"?"Yarın gece vardiyasındasın 🌙":map.category==="off"?"Yarın izinlisin":map.category==="annual"?"Yarın yıllık izindesin":`Yarın: ${map.label}`;const title=predicted?`Tahmine göre: ${baseTitle}`:baseTitle;await self.registration.showNotification(title,{body:`${predicted?"Tahmini program • Excel gelince doğrulayın • ":""}Kod: ${code}${map.start?" • "+map.start+" başlangıç":""}`,icon:"icons/icon-192.png",badge:"icons/icon-192.png",tag:"vardiyacep-reminder"});await put(key,true);}
